    ...
    data?.floorConsumptions?.forEach(({ floorNumber,
        ...
        floorDetails.forEach(({ energyConsumedKwh, floorId, timestamp }: { energyConsumedKwh: string, floorId: number, timestamp: string }) => {
            datasetNew.push({
                ...
                floor: floorNumber,
                time_windlow: dayjs(new Date(timestamp)).format('YYYY-MM-DD hh:mm A')
            })
        })
    });
...
export const normalizeBuildingFloorwiseData = (data: any, isFloorwiseData: boolean) => {
    let dataset = []
    if (isFloorwiseData) {
        let floorDetails: any[] = data?.floorConsumptions?.[0]?.floorDetails?.sort((a: any, b: any) => (
            new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
        ))
        dataset = floorDetails?.map(({ energyConsumedKwh, timestamp }: {
...
export const normalizeBuildingOccupancy = (data: any, isFloorwiseData: boolean) => {
    console.log(data)
