...
export const periodCalculatorForLastSelector = (value: string): { start: Date, end: Date } => {
    let start = new Date()
    let end = new Date()
...
const calculateQuarter = (isCurrentQuarter: boolean): { start: Date, end: Date } => {
    ...
    if (currentMonth >= 0 && currentMonth <= 2) { // Q1
        ...
        if (!isCurrentQuarter)
            today.setFullYear(today.getFullYear() - 1); // Move back to the previous year
...
const calculateHalfYear = (isCurrentHalfYear: boolean): { start: Date, end: Date } => {
    let today = new Date();
    let currentMonth = today.getMonth();
    let start, end

    if (currentMonth >= 0 && currentMonth <= 5) { // Current month is in the first half (Jan-Jun)
    ...
    } else { // Current month is in the second half (Jul-Dec)
...
export const periodCalculatorForRelativeSelector = (value: string): { start: Date, end: Date } => {
    let start = new Date()
    let end = new Date()
    switch (value) {
        case "yesterday":
