...
import { historyTimefilters, lastFilterOptions, relativeFilterOptions, relativeTimefilters, timeFilters } from "../../constants/staticDateFilters"
import dayjs, { Dayjs } from "dayjs"
import { DateFilterTypes } from "./types"
import './DateFilter.scss'
import Selector from "./components/Selector"
import DateTimeSelector from "./components/DateTimeSelector"
import { periodCalculatorForLastSelector, periodCalculatorForRelativeSelector } from "../../utils/dateFiltersUtils"

const DateFilters = ({ onDateRangeChange, defaultStartDate, defaultEndDate }: DateFilterTypes) => {
    ...
    const handleTimeFiltersOpen = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const updateBtnLabel = () => {
        ...
        switch (activeView) {
            case 'last':
                text = lastFilterOptions.find((option) => option.value === lastSelector)?.label ?? ''
                break
            case 'relative':
                text = relativeFilterOptions.find((option) => option.value === relativeSelector)?.label ?? ''
                break
            default:
                text = `from ${startDate?.format('DD/MM/YYYY')} to ${endDate?.format('DD/MM/YYYY')}`
        }
        let label = `${timeFilterValue === 'relative' ? 'Relative' : 'History'} - ${text}`
        setBtnLabel(label)
    }
    const handleUpdate = () => {
        setAnchorEl(null)
                ...
                <Paper sx={{
                    ...
                    <Box
                        ...
                        <ToggleButtonSelector
                            ...
                            variant='default'
                            value={activeView}
                            options={timeFilterValue === 'relative' ? relativeTimefilters : historyTimefilters}
                        />
                        {activeView === 'last' && (
                            <Selector
                    ...
                    <Box sx={{
                        ...
                        <Button
                            sx={{ fontSize: '14px', textTransform: 'unset', color: '#406D91' }}
