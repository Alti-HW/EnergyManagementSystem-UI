...
const BuildingCostChart = ({ startDate, endDate }: ChartProps) => {
  ...
  useEffect(() => {
    const { dataset, totalcost: cost } = normalizeBuildingCostData(
      buildingsRawData?.data
    );
  ...
  const renderChart = (fullscreen?: boolean) => (
    <div   ref={chartRef}>
      {isBuildingsDataLoading ? (
        ...
        <PieChart
          series={[
            { data: buildingsCostData, innerRadius: fullscreen ? 220 : 70 },
          ]}
        ...
        >
          {buildingsCostData?.length > 0 && (
            <PieCenterLabel>Total: {Math.round(totalcost)}</PieCenterLabel>
          )}
        </PieChart>
      )}
    </div>
