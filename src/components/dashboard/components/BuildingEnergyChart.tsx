...
import { normalizeAllBuildingsData, normalizeBuildingDrilldownData } from "../../../utils/normalizeDashboardAPIData";
...
const BuildingEnergyChart = ({ startDate, endDate }: ChartProps) => {
    ...
    useEffect(() => {
        let data = normalizeBuildingDrilldownData(buildingsDrillDownRawData?.data?.[0])
    ...
    const renderChart = (fullscreen?: boolean) => (
        <>
            {isBuildingsDataLoading ? <Spinner /> : buildingsDataError ? <ErrorMessage /> :
                <>
                    <BarChart
                        xAxis={[{
                            scaleType: 'band',
